...
**Date:** 15 April 2026  
**Files reviewed:** `partial_association_explorer_7.r`, `paper/draft/Association_explorer_methods.tex`  
...
✓ The implementation in `partial_residuals()` regresses each variable on the controls independently and then correlates the two residual series (`partial_association_explorer_7.r:1467-1482`, `2585-2587`). This is the correct added-variable / Frisch-Waugh approach as described in Section 2 of the paper. The t-statistic in `p_value_partial_cor()` uses `df = n_eff - k_controls - 2`, matching the paper's formula (`partial_association_explorer_7.r:1703`). No error here, but there is a subtle counting issue: `k_controls` is set to `ncol(control_data)` before any variation-filtering (`partial_association_explorer_7.r:2594`), so if some controls are constant and get dropped inside `partial_residuals()` (`partial_association_explorer_7.r:1473-1474`), the degrees of freedom used for the p-value will be overestimated (too many df removed), making the test conservative. Fix: pass the number of controls *actually used* (post-filtering) back alongside the residuals.
...
- ✓ `safe_g2_cell()` is referenced inside the fallback path in `compute_unconditional()` (`partial_association_explorer_7.r:2299`) but is **never defined** anywhere in the file. If `fit0` fails for any contingency table, the fallback will throw a `could not find function "safe_g2_cell"` error. This is a definite bug.

- ✓ In `compute_conditional()`, the result list renames `VL` to `VL/Z` at the very end (`partial_association_explorer_7.r:2522`). Downstream code accesses `cor_result[["VL/Z"]]` (`partial_association_explorer_7.r:2632`) in the pairs plot and `vl_value <- assoc_res[["VL/Z"]]` (`partial_association_explorer_7.r:1078`) in the correlation matrix. This is consistent but brittle; a slash in a list name is legal in R but unusual and easy to break. Rename it to something like `VL_Z` throughout.
...
✓ The association measure displayed in the network tooltip uses the raw matrix value (which can be `r` or `sqrt(eta2)` depending on case), while the network summary block labels edges as "r" for numeric-numeric. This is fine but the threshold slider is labeled "R²" in the UI (`partial_association_explorer_7.r:108`, full label: `"Threshold for Quantitative-Quantitative and Quantitative-Categorical Associations (R²)"`). The slider filters on `r^2 >= threshold_num` for numeric-numeric (`partial_association_explorer_7.r:2590`) but on `eta_sq >= threshold_num` for mixed cases (`partial_association_explorer_7.r:2673`). The label "R²" is therefore misleading for mixed pairs, where the quantity being thresholded is eta-squared, not R-squared. The slider label should read something like "R² / η² threshold".
...
1. ✓ **`safe_g2_cell` undefined** (critical): see Section 1.1 above. Called at `partial_association_explorer_7.r:2299`, never defined anywhere in the file.
...
6. ✓ **`compute_conditional` fallback**: when `fit0` fails (`partial_association_explorer_7.r:2448-2451`), the code falls back to `compute_unconditional()` silently. The returned list will have `VL` named `VL` (not `VL/Z`), but the downstream renaming (`names(out)[names(out) == "VL"] <- "VL/Z"`) only applies to the conditional success path (`partial_association_explorer_7.r:2522`). So on fallback, the association matrix code will call `cor_result[["VL/Z"]]` (`partial_association_explorer_7.r:2632`) and get `NULL`, storing `0`. This will make the pair silently disappear from the network rather than falling back to the unconditional value. Fix: ensure the fallback also renames `VL` to `VL/Z` in the conditional function.

> NUANCE: There are **three** un-renamed fallback paths in `compute_conditional()`, not just one: (a) empty `Zdf` (`partial_association_explorer_7.r:2402-2403`), (b) `fit0` failure (`partial_association_explorer_7.r:2448-2451`), and (c) `fit1` failure (`partial_association_explorer_7.r:2480-2497`, returns `make_catcat_result(VL = NA_real_, ...)` also not renamed). All three silently return `NULL` for `cor_result[["VL/Z"]]`.
...
- ✓ **Section 2.1, unconditional case**: The paper defines `R2_XY = r_XY^2` (`Association_explorer_methods.tex:95`) and says it is used as the global symmetric association measure. The code stores `abs(r)` in the correlation matrix (`partial_association_explorer_7.r:2590`: `cor_val <- ifelse(r^2 >= threshold_num, abs(r), 0)`) rather than `r^2`. The threshold slider is labeled R² but compared as `r^2 >= threshold_num`. So the value plotted in the network tooltip is `|r|`, not `r^2`. The paper should clarify which quantity is displayed where, or the code should be made consistent (store `r^2` and compare `r^2 >= threshold`).
...
- ✓ The paper describes interaction parameters `gamma_ij` as being displayed in the pair plots alongside D and R. The code does include a gamma table in the UI (`partial_association_explorer_7.r:1154-1191`). **However**, the gamma matrix returned by `fit1$params$gamma` is a full I×J matrix including the reference row/column (which are always 0 by constraint, `partial_association_explorer_7.r:1997-2003`). The paper's notation indexes gamma only for non-reference cells. The displayed table converts the full matrix directly (`partial_association_explorer_7.r:1155`) and therefore shows a row and column of zeros corresponding to the reference category, which could confuse users. Add a note in the UI or trim the reference row/column from the displayed gamma table.
...
12. **Fix the df overcounting** in `p_value_partial_cor()` when some controls have been dropped during variation filtering (`partial_association_explorer_7.r:2594` vs `partial_residuals()` line 1473).
...
18. Trim reference rows/columns from the displayed gamma table in the cat-cat pair plots (`partial_association_explorer_7.r:1155`).
19. Clarify the slider label "R²" to read "R² / η² threshold" for mixed pairs (`partial_association_explorer_7.r:108`).